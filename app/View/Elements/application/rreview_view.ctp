...
<?php if ($hasComparison) { ?>
  <style type="text/css">
    ...
    .manager-reviewer2-block {
      border-left: 4px solid #b30000;
      background: #fff1f1;
      color: #b30000;
      padding: 8px;
    }
    .manager-reviewer1-block p,
    .manager-reviewer2-block p {
      margin: 0;
    }
  </style>
  <div class="alert alert-info" style="margin-bottom: 10px;">
    Linked responses are color coded when both reviewers provided an answer:
    <strong><?php echo h($comparisonReviewerName); ?> (blue)</strong>,
    <strong><?php echo h($currentReviewerName); ?> (red)</strong>.
  ...
  <tbody>
    ...
    for ($i = 0; $i <= count($rreview['ReviewAnswer']) - 1; $i++) {
      ...
      if ($hasComparison && $previousAnswerText !== '' && $currentAnswerText !== '') {
        echo '<div class="manager-reviewer1-block"><strong>' . h($comparisonReviewerName) . ':</strong><br>' . $formatAnswerText($previousAnswerText) . '</div>';
        echo '<div class="manager-reviewer2-block"><strong>' . h($currentReviewerName) . ':</strong><br>' . $formatAnswerText($currentAnswerText) . '</div>';
