...
class StudyAuditorsController  extends AppController
{
    public $paginate = array();
   public $uses = array('StudyAuditor', 'User', 'Application');  
    ...
    public function admin_index()
    {
        ...
        $criteria['User.group_id'] = 10; 
        $this->paginate['User']['conditions'] = $criteria;
        $this->paginate['User']['order'] = array('User.created' => 'desc');
      $this->paginate['User']['contain'] = array('Group', 'StudyAuditor' => array('Application'));  
    ...
    public function admin_view($user_id = null, $application_id = null)
    {
        ...
        $user = $this->User->find('first', array(
            'contain' => array('StudyAuditor' => array('Application')),
        ...
        $criteria['NOT'] = array('Application.id' => Hash::extract($user['StudyAuditor'], '{n}.application_id'));
        ...
        if ($this->request->is('post') && !empty($application_id)) {
            ...
            if ($this->StudyAuditor->save($saveData)) {
    ...
    public function admin_delete($id = null)
    {
        ...
        $this->StudyAuditor->id = $id;
        if (!$this->StudyAuditor->exists()) {
            throw new NotFoundException(__('Invalid assignment record.'));
        }

        if ($this->StudyAuditor->delete()) {
